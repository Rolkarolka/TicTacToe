...
class TicTacToe(Graphic):
    ...
    def choose_next_AI_step(self, current_heurisitic, node):  # choose best movement for AI
        children = []
        terminal = []
        ...
        while self.run:
            for event in pg.event.get():
                if self.whoiswho[self.player] == 'H' and event.type == pg.MOUSEBUTTONUP:
                    ...
                    max_val = minimax(node, 8, self.player)
                    child = self.choose_next_AI_step(max_val, node)
                    self.board = child.current_board
                    coords, self.result = self.check_conditions(int(self.player), self.board)
                    self.player = int(not self.player)
